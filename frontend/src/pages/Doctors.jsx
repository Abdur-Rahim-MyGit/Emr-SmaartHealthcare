...
import { RiMentalHealthLine, RiStethoscopeLine } from 'react-icons/ri'
import { HiOutlineSearch, HiOutlineAdjustments, HiOutlineClock } from 'react-icons/hi'
...
const Doctors = ({ openAppointmentModal }) => {
  ...
  // Sort and filter doctors safely
  const getSortedAndFilteredDoctors = () => {
    const searchLower = searchTerm.toLowerCase()

    let filtered = doctors.filter(doctor => {
      const doctorName = doctor.name ? doctor.name.toLowerCase() : ''
      const doctorSpeciality = doctor.speciality ? doctor.speciality.toLowerCase() : ''

      const matchesSearch =
        doctorName.includes(searchLower) ||
        doctorSpeciality.includes(searchLower)

      const matchesSpecialty = !specialty || specialty === '' || doctor.speciality === specialty
          ...
          {filteredDoctors.map((doctor) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-primary font-medium mt-1">{doctor.speciality}</p>
